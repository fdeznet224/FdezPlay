...
class MobileSeriesPlayerScreen extends StatefulWidget {
  const MobileSeriesPlayerScreen({
    required this.session,
    required this.series,
    required this.seasonName,
    required this.episodes,
    required this.initialIndex,
    this.playbackVersions = const [],
    this.initialVersionIndex = 0,
    this.displayTitle,
    this.initialPosition = Duration.zero,
    this.offlineEpisodePaths = const {},
  ...
  final List<SeriesPlaybackVersion> playbackVersions;
  final int initialVersionIndex;
  final String? displayTitle;
  final Duration initialPosition;
  final Map<int, String> offlineEpisodePaths;
  final bool enableTvRemoteNavigation;

  @override
  State<MobileSeriesPlayerScreen> createState() {
    return _MobileSeriesPlayerScreenState();
  }
}

class _MobileSeriesPlayerScreenState
    extends State<MobileSeriesPlayerScreen>
    with WidgetsBindingObserver {
  final FocusNode _remotePlayerFocusNode = FocusNode(
    debugLabel: 'series-player-root',
  );
  final FocusNode _backButtonFocusNode = FocusNode(
    debugLabel: 'series-player-back',
  );
  final FocusNode _previousButtonFocusNode = FocusNode(
    debugLabel: 'series-player-previous',
  );
  final FocusNode _playPauseFocusNode = FocusNode(
    debugLabel: 'series-player-play-pause',
  );
  final FocusNode _nextButtonFocusNode = FocusNode(
    debugLabel: 'series-player-next',
  );
  final FocusNode _audioButtonFocusNode = FocusNode(
    debugLabel: 'series-player-audio',
  );
  final FocusNode _subtitleButtonFocusNode = FocusNode(
    debugLabel: 'series-player-subtitle',
  );
  final FocusNode _castButtonFocusNode = FocusNode(
    debugLabel: 'series-player-cast',
  );
  final FocusNode _seekBarFocusNode = FocusNode(
    debugLabel: 'series-player-seek-bar',
  );

  late VlcPlayerController _controller;
  late int _currentIndex;
  late Duration _resumePosition;
  ...
  List<_EpisodeSource> _sourceCandidates = const [];
  int _sourceIndex = 0;
  ...
  SeriesEpisode get _currentEpisode => widget.episodes[_currentIndex];

  bool get _hasPreviousEpisode => _currentIndex > 0;

  bool get _hasNextEpisode => _currentIndex < widget.episodes.length - 1;

  _EpisodeSource get _activeSource => _sourceCandidates[_sourceIndex];

  String? get _activeOfflinePath {
    final path = widget.offlineEpisodePaths[_activeSource.episode.episodeId];
  ...
  bool get _isActiveEpisodeOffline => _activeOfflinePath != null;

  String get _episodeUrl => _activeOfflinePath ??
      _buildEpisodeUrl(_activeSource.episode);

  String _buildEpisodeUrl(SeriesEpisode episode) {
  ...
  void _resetSourceCandidates() {
    final logicalEpisode = _currentEpisode;
    final versions = List<SeriesPlaybackVersion>.from(
      widget.playbackVersions,
    );
    final orderedVersions = <SeriesPlaybackVersion>[];

    if (versions.isNotEmpty) {
      final preferred = widget.initialVersionIndex
          .clamp(0, versions.length - 1)
          .toInt();
      orderedVersions.add(versions[preferred]);

      for (int index = 0; index < versions.length; index++) {
        if (index != preferred) {
          orderedVersions.add(versions[index]);
        }
      }
    }

    final candidates = <_EpisodeSource>[];
    final seen = <int>{};

    for (final version in orderedVersions) {
      SeriesEpisode? matchingEpisode;

      for (final season in version.details.seasons) {
        if (season.seasonNumber != logicalEpisode.seasonNumber) {
          continue;
        }

        for (final episode in season.episodes) {
          if (episode.episodeNumber == logicalEpisode.episodeNumber) {
            matchingEpisode = episode;
            break;
          }
        }

        matchingEpisode ??= season.episodes.isEmpty
            ? null
            : season.episodes[
                _currentIndex.clamp(0, season.episodes.length - 1).toInt()
              ];
        break;
      }

      if (matchingEpisode != null && seen.add(matchingEpisode.episodeId)) {
        candidates.add(
          _EpisodeSource(
    ...
    if (seen.add(logicalEpisode.episodeId)) {
      candidates.add(
        _EpisodeSource(
    ...
    _sourceCandidates = List<_EpisodeSource>.unmodifiable(candidates);
    _sourceIndex = 0;
  ...
  String get _playerTitle {
    ...
    final seriesTitle = title.isEmpty ? widget.series.name : title;

    return '$seriesTitle • ${widget.seasonName} • $episodeLabel';
  }
  ...
  VlcPlayerController _buildController() {
    ...
    final offlinePath = _activeOfflinePath;
  ...
  void initState() {
    ...
    _currentIndex = widget.initialIndex
        .clamp(0, widget.episodes.length - 1)
        .toInt();
    _resumePosition = widget.initialPosition;
    _resetSourceCandidates();
  ...
  Future<void> _initializePlayer() async {
    ...
    try {
      ...
      unawaited(_scheduleMediaTrackLoad());
    } on TimeoutException catch (error) {
      await _handleSourceFailure(
        'VLC tardó demasiado en abrir el episodio.',
        error: error,
      );
    } catch (error) {
      await _handleSourceFailure(
  ...
  Future<void> _handlePlaybackInstability() async {
    ...
    await _handleSourceFailure(
      'La conexión del episodio se interrumpió.',
    );
  }
  ...
  Future<void> _checkNetworkRecovery() async {
    ...
    if (!recovered) {
      await _handleSourceFailure(
        'La conexión volvió, pero el episodio no respondió.',
      );
    }
  }
    ...
    try {
      ...
      if (stabilize) {
        ...
        try {
          ...
          debugPrint(
            'No fue posible silenciar el episodio durante la estabilización: $error',
          );
        }
      }
      ...
      debugPrint('No fue posible recrear VLC para la episodio: $error');
      return false;
    } finally {
      _resettingController = false;
    }
  ...
  Future<void> _checkRecoveryWarmup() async {
    ...
    try {
      ...
      final position = value.position;
      final now = DateTime.now();
      final resumeReady = _resumePosition <= const Duration(seconds: 5) ||
          _resumeApplied;
      final advanced =
      ...
      if (timedOut) {
        if (value.isPlaying && resumeReady) {
          ...
          _cancelRecoveryWarmup();
          await _handleSourceFailure(
            'El episodio no logró estabilizarse.',
          );
        }
      }
    } finally {
  ...
  Future<void> _finishRecoveryWarmup() async {
    ...
    try {
      ...
      debugPrint('No fue posible restaurar el audio del episodio: $error');
    }
  ...
  Future<bool> _reconnectCurrentSource({bool hardReset = false}) async {
    ...
    try {
      ...
      debugPrint('No fue posible reconectar el episodio: $error');
      return false;
    } finally {
      _reconnecting = false;
    }
  ...
  Future<bool> _switchToNextSource() async {
    if (_isActiveEpisodeOffline || _isClosing || _switchingSource) {
      return false;
    }
    ...
    try {
      while (_sourceIndex + 1 < _sourceCandidates.length) {
        _sourceIndex++;
        _resumeApplied = false;
        _resumeInProgress = false;
        ...
        try {
          ...
          debugPrint(
            'Versión de episodio ${_activeSource.episode.episodeId} '
            'no disponible: $error',
          );
        }
      }
    ...
    try {
      for (int attempt = 0; attempt < 48; attempt++) {
        ...
        if (value.isInitialized &&
          ...
          if (!mounted || _isClosing || _changingEpisode) {
            return;
          }
      ...
      debugPrint(
        'VLC no confirmó la posición guardada del episodio.',
      );
    } catch (error) {
      debugPrint(
        'Error reanudando el episodio: $error',
      );
    } finally {
      _resumeInProgress = false;
    }
  ...
  void _onPlayerChanged() {
      ...
      unawaited(
        _handleSourceFailure(
          value.errorDescription.trim().isEmpty
              ? 'No fue posible reproducir el episodio.'
    ...
    if (value.isEnded && !_ended && !_changingEpisode) {
      unawaited(
        _libraryService.removeEpisodeProgress(
          widget.session,
          _currentEpisode.episodeId,
        ),
      );

      if (_playbackPreferences.autoPlayNextEpisode &&
          _hasNextEpisode) {
        _ended = true;
        unawaited(_changeEpisode(_currentIndex + 1));
        return;
      }
  ...
  Future<void> _retry() async {
    ...
    await _openCurrentEpisode();
  }

  Future<void> _openCurrentEpisode() async {
    if (_isClosing) {
      return;
    }
  ...
  Future<void> _changeEpisode(int newIndex) async {
    if (_isClosing ||
        _changingEpisode ||
        newIndex < 0 ||
        newIndex >= widget.episodes.length ||
  ...
  Future<void> _scheduleMediaTrackLoad() async {
  ...
  Future<void> _loadMediaTracks() async {
    ...
    try {
      ...
      for (int attempt = 0; attempt < 8; attempt++) {
        if (_isClosing || _changingEpisode) {
          return;
        }
        ...
        try {
          ...
          debugPrint('Esperando pistas del episodio: $error');
        }
      ...
      debugPrint('Error cargando pistas del episodio: $error');
    } finally {
  ...
  Future<void> _showAudioTracks() async {
    ...
    if (_audioTracks.isEmpty) {
      _showMessage('Este episodio no informa pistas de audio adicionales.');
      return;
    }
  ...
  Future<void> _showSubtitleTracks() async {
    ...
    if (availableSubtitles.isEmpty) {
      _showMessage('Este episodio no contiene subtítulos integrados.');
      return;
    }
  ...
  Future<void> _showCastDevices() async {
    ...
    final media = FdezCastMedia(
      title: _playerTitle,
      url: _episodeUrl,
      mimeType: fdezCastMimeTypeFromUrl(_episodeUrl),
  ...
  Future<void> _togglePlayback() async {
    ...
    if (_ended || _controller.value.isEnded) {
      _playbackExpected = true;
      _resetPlaybackWatchdog();
  ...
  Future<void> _seekTo(Duration requestedPosition) async {
    ...
    if (!value.isInitialized || duration <= Duration.zero) {
      _showMessage('El episodio todavía no está listo para adelantar.');
      return;
    }
    ...
    try {
      ...
      if (seekable != true) {
        _showMessage('Este episodio no permite adelantar desde el servidor.');
        return;
      }
      ...
      debugPrint('Error adelantando episodio: $error');
      _showMessage('No fue posible adelantar este episodio.');
  ...
  Future<void> _saveProgressIfNeeded(
    ...
    if (_savingProgress || _seekInProgress || _ended || _changingEpisode) {
      return;
    }
    ...
    try {
      await _libraryService.saveEpisodeProgress(
  ...
  Future<void> _handleBackground() async {
    ...
    try {
      ...
      debugPrint('No fue posible pausar el episodio al minimizar: $error');
    }
  }
  ...
  Future<void> _handleForeground() async {
    ...
    try {
      ...
      await _handleSourceFailure(
        'No fue posible reanudar el episodio.',
        error: error,
      );
    }
  }
  ...
  Future<void> _closePlayer() async {
    ...
    try {
      ...
      debugPrint('VLC ya estaba cerrándose: $error');
    }
  ...
  Future<void> _disposePlayer() async {
    ...
    try {
      ...
      debugPrint('VLC ya estaba detenido: $error');
    }

    try {
      await _controller.dispose();
    } catch (error) {
      debugPrint('VLC ya estaba liberado: $error');
    }
  ...
  void dispose() {
    ...
    _previousButtonFocusNode.dispose();
  ...
  KeyEventResult _handleRemotePlayerKey(
    ...
    final controlHasFocus = _backButtonFocusNode.hasFocus ||
        _previousButtonFocusNode.hasFocus ||
    ...
    if (isDirectionalKey) {
      ...
      if (key == LogicalKeyboardKey.arrowUp) {
        if (_seekBarFocusNode.hasFocus) {
          _playPauseFocusNode.requestFocus();
        } else if (_previousButtonFocusNode.hasFocus ||
            _playPauseFocusNode.hasFocus ||
            _nextButtonFocusNode.hasFocus) {
      ...
      if (key == LogicalKeyboardKey.arrowLeft) {
        ...
        } else if (_nextButtonFocusNode.hasFocus) {
          _playPauseFocusNode.requestFocus();
        }
        return KeyEventResult.handled;
      }
    ...
    if (key == LogicalKeyboardKey.mediaTrackPrevious && _hasPreviousEpisode) {
      unawaited(_changeEpisode(_currentIndex - 1));
      return KeyEventResult.handled;
    }

    if (key == LogicalKeyboardKey.mediaTrackNext && _hasNextEpisode) {
      unawaited(_changeEpisode(_currentIndex + 1));
      return KeyEventResult.handled;
    }
                ...
                AnimatedOpacity(
                  ...
                  child: IgnorePointer(
                    ...
                    child: _SeriesControls(
                      ...
                      previousFocusNode: _previousButtonFocusNode,
                      playPauseFocusNode: _playPauseFocusNode,
                      seekBarFocusNode: _seekBarFocusNode,
                      nextFocusNode: _nextButtonFocusNode,
            ...
            child: Column(
              children: [
                ...
                ValueListenableBuilder<VlcPlayerValue>(
                  ...
                  builder: (context, value, child) {
                    final duration = value.duration;
                    final position = seekPreviewPosition ?? value.position;
                    final icon = ended
              ...
              child: Row(
                children: [
                  Container(
                    ...
                    child: Icon(icon, color: const Color(0xFFFFA66B)),
                  ),
                  const SizedBox(width: 12),
                  Expanded(
                    child: Text(
...
class _SeriesPlayerError extends StatelessWidget {
  const _SeriesPlayerError({
    required this.message,
...
String _formatDuration(Duration value) {
  final totalSeconds = value.inSeconds.clamp(0, 359999).toInt();
