...
class _TvLiveTvScreenState extends State<TvLiveTvScreen> {
  ...
  void initState() {
    ...
    unawaited(_loadFavoriteChannelsDeferred());
  }
  ...
  Future<void> _loadFavoriteChannelsDeferred() async {
  ...
  List<LiveChannelGroup>? _cachedGroupsForCategory(String categoryId) {
    final cachedGroups = _searchIndexService.cachedChannelGroups;
  ...
  List<LiveChannelGroup>? _cachedAllGroups() {
    final cachedGroups = _searchIndexService.cachedChannelGroups;
    if (cachedGroups == null || cachedGroups.isEmpty) {
      return null;
    }

    return List<LiveChannelGroup>.unmodifiable(cachedGroups);
  }
  ...
  Future<void> _loadInitialData() async {
    ...
    try {
      ...
      final cachedGroups = _cachedGroupsForCategory(firstCategory.id);
      final groups = cachedGroups ??
          groupLiveChannels(
  ...
  Future<void> _loadAllChannelsForSearch({bool force = false}) async {
    ...
    try {
      final cachedGroups = _cachedAllGroups();
      final groups = cachedGroups ??
          (await _searchIndexService.ensureChannelIndex(widget.session))
  ...
  Future<void> _selectCategory(IptvCategory category) async {
    ...
    try {
      final cachedGroups = _cachedGroupsForCategory(category.id);
      final groups = cachedGroups ??
          groupLiveChannels(
            await _service.loadChannels(
              session: widget.session,
  ...
  Future<void> _refreshVisibleContent() async {
    ...
    try {
      final groups = groupLiveChannels(
        await _service.loadChannels(
          session: widget.session,
  ...
  Widget build(BuildContext context) {
    if (_loadingInitialData) {
      return const _TvTvInitialLoading();
    }

    if (_errorMessage != null && _categories.isEmpty) {
      return _TvTvFullError(
...
class _TvTvHeader extends StatelessWidget {
  const _TvTvHeader({
...
class _TvTvSearchField extends StatelessWidget {
  const _TvTvSearchField({
...
class _TvCategoryPanel extends StatelessWidget {
...
class _TvChannelPanel extends StatelessWidget {
  const _TvChannelPanel({
  ...
  Widget _buildContent() {
    if (_loading) {
      return const _TvPanelLoading(
        message: 'Cargando canales...',
      );
    }

    if (errorMessage != null) {
      return _TvPanelError(
...
class _TvChannelDetailsPanel extends StatelessWidget {
  const _TvChannelDetailsPanel({
    required this.group,
    required this.categoryName,
    required this.favorite,
...
class _TvPanel extends StatelessWidget {
  const _TvPanel({required this.child});
...
class _TvPanelLoading extends StatelessWidget {
  const _TvPanelLoading({required this.message});
...
class _TvPanelError extends StatelessWidget {
  const _TvPanelError({
...
class _TvPanelEmpty extends StatelessWidget {
  const _TvPanelEmpty({
    required this.icon,
    required this.title,
...
class _TvTvInitialLoading extends StatelessWidget {
  const _TvTvInitialLoading();
...
class _TvTvFullError extends StatelessWidget {
  const _TvTvFullError({
    required this.message,
    required this.onRetry,
  });
