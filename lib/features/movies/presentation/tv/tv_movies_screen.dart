...
class _TvMoviesScreenState extends State<TvMoviesScreen> {
  ...
  List<Movie>? _cachedMoviesForCategory(String categoryId) {
    final cachedGroups = _searchIndexService.cachedMovieGroups;
    ...
    return cachedGroups
        .where(
          (group) => group.variants.any((movie) => movie.categoryId == categoryId),
        )
        .expand((group) => group.variants)
        .toList(growable: false);
  }

  List<Movie>? _cachedAllMovies() {
    final cachedGroups = _searchIndexService.cachedMovieGroups;
    if (cachedGroups == null || cachedGroups.isEmpty) {
      return null;
    }
  ...
  Future<void> _loadInitialData() async {
    ...
    try {
      ...
      final movies = _cachedMoviesForCategory(firstCategory.id) ??
          await _apiService.loadMovies(
            widget.session,
  ...
  Future<void> _selectCategory(IptvCategory category) async {
    ...
    try {
      final movies = _cachedMoviesForCategory(category.id) ??
          await _apiService.loadMovies(
            widget.session,
  ...
  Future<void> _loadAllMoviesForSearch({bool force = false}) async {
    ...
    try {
      final movies = !force
          ? (_cachedAllMovies() ??
              (await _searchIndexService.ensureMovieIndex(widget.session))
  ...
  Widget build(BuildContext context) {
    if (_loadingInitialData) {
      return const _TvMoviesInitialLoading();
    }

    if (_errorMessage != null && _categories.isEmpty) {
      return _TvMoviesFullError(
...
class _TvMoviesHeader extends StatelessWidget {
  const _TvMoviesHeader({
...
class _TvMoviesSearchField extends StatelessWidget {
  const _TvMoviesSearchField({
...
class _TvMovieCategoryPanel extends StatelessWidget {
  const _TvMovieCategoryPanel({
...
class _TvMovieCatalogPanel extends StatelessWidget {
  const _TvMovieCatalogPanel({
  ...
  Widget _buildContent() {
    if (_loading) {
      return const _TvMoviePanelLoading(
        message: 'Cargando películas...',
      );
    }

    if (errorMessage != null) {
      return _TvMoviePanelError(
...
class _TvMovieCard extends StatelessWidget {
  const _TvMovieCard({
    required this.group,
                  ...
                  children: [
                    ClipRRect(
                      ...
                      child: movie.posterUrl.isEmpty
                          ? const _TvMoviePosterFallback()
                          : AppCachedImage(
                              imageUrl: movie.posterUrl,
                              fit: BoxFit.cover,
                              cacheWidth: 300,
                              cacheHeight: 450,
                              placeholder: const _TvMoviePosterFallback(
                                loading: true,
                              ),
                              fallback: const _TvMoviePosterFallback(),
                            ),
                    ),
                    if (favorite)
                      const Positioned(
...
class _TvMovieDetailsPanel extends StatelessWidget {
  const _TvMovieDetailsPanel({
    required this.group,
    required this.movie,
  ...
  Widget build(BuildContext context) {
    ...
    if (selectedGroup == null || selectedMovie == null) {
      return const _TvMoviePanel(
        child: _TvMoviePanelEmpty(
                        ...
                        children: [
                          if (backgroundUrl.isNotEmpty)
                            AppCachedImage(
                              ...
                              cacheWidth: 950,
                              cacheHeight: 520,
                              fallback: const _TvMovieBackdropFallback(),
                            )
                          else
                            const _TvMovieBackdropFallback(),
                          const DecoratedBox(
                            decoration: BoxDecoration(
...
class _TvMovieInfoChip extends StatelessWidget {
  const _TvMovieInfoChip({
    required this.icon,
    required this.label,
    this.accent = false,
  });
...
class _TvMovieDetailLine extends StatelessWidget {
  const _TvMovieDetailLine({
    required this.label,
    required this.value,
  });
...
class _TvMoviePanel extends StatelessWidget {
  const _TvMoviePanel({required this.child});
...
class _TvMovieBadge extends StatelessWidget {
  const _TvMovieBadge({
    required this.icon,
    required this.color,
    this.text,
...
class _TvMoviePosterFallback extends StatelessWidget {
  const _TvMoviePosterFallback({this.loading = false});
...
class _TvMovieBackdropFallback extends StatelessWidget {
  const _TvMovieBackdropFallback();
...
class _TvMoviePanelLoading extends StatelessWidget {
  const _TvMoviePanelLoading({required this.message});
...
class _TvMoviePanelError extends StatelessWidget {
  const _TvMoviePanelError({
...
class _TvMoviePanelEmpty extends StatelessWidget {
  const _TvMoviePanelEmpty({
    required this.icon,
    required this.title,
...
class _TvMoviesInitialLoading extends StatelessWidget {
  const _TvMoviesInitialLoading();
...
class _TvMoviesFullError extends StatelessWidget {
  const _TvMoviesFullError({
    required this.message,
    required this.onRetry,
  });
